...
def tabular_ext_learner(data:DataBunch, layers:Collection[int], vec_szs:Dict[str,int]=None, emb_szs:Dict[str,int]=None, metrics=None,
        ps:Collection[float]=None, emb_drop:float=0., y_range:OptRange=None, use_bn:bool=True, **learn_kwargs):
    "Get a `Learner` using `data`, with `metrics`, including a `TabularModel` created using the remaining params."
    emb_szs = data.get_emb_szs(ifnone(emb_szs, {}))
    vec_szs = data.get_vec_szs(ifnone(vec_szs, {}))
    model = TabularExtModel(vec_szs, emb_szs, len(data.cont_names), out_sz=data.c, layers=layers, ps=ps, emb_drop=emb_drop,
