...
# +
# def create_vec_map(column_n):
#     vec_value_set = {}
#     for column_value_list in column_n:
#         for column_value in column_value_list:
#             vec_value_set.add(column_value)
#     vec_value_list = list(vec_value_set)
#     vec_map = {}
#     index = 0          # already accounted for nan
#     for vec in vec_value_list:
#         vec_map[vec] = index
#         index += 1    
...
# +
# def derive_map_vector(list_values, vec_map):
#     list_vec = [0] * len(vec_map)
#     for list_value in list_values:
#         try:
#             vec_index = vec_map[list_value]
#             list_vec[vec_index] = list_vec[vec_index] + 1
#         except:
...
# +
# class Sparsify(TabularProc):
#     "Transform the list variables to a sparse vector."
#     def apply_train(self, df:DataFrame):
#         "Transform `self.cat_names` columns in categorical."
#         self.vecs = {}
#         for n in self.vec_names:
#             if n == 'SKU_vector':
#                 pass    # do nothing. already transformed
#             else:  # transform to sparse vector
#                 print('df.loc[:,{}]={}'.format(n, df.loc[:,n]))

#                 df.loc[:,n] = df.loc[:,n].astype('category').cat.as_ordered()
#                 self.vecs[n] = df[n].cat.vecs
#                 print('self.categories[{}] = {}'.format(n, self.categories[n]))
