...
class LRFinder(LearnerCallback):
    ...
    def on_batch_end(self, iteration:int, smooth_loss:TensorOrNumber, **kwargs:Any)->None:
        ...
        if self.sched.is_done or (self.stop_div and (smooth_loss > 4*self.best_loss or torch.isnan(smooth_loss))):
            print('self.sched.is_done={}'.format(self.sched.is_done))
            print('self.stop_div={}'.format(self.stop_div))
            print('smooth_loss={}'.format(smooth_loss))
            print('self.best_loss={}'.format(self.best_loss))
            print('torch.isnan(smooth_loss)={}'.format(torch.isnan(smooth_loss)))
